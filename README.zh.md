...
客户端后续接入时，应拒绝不支持的目录版本，按目录记录的提交安装，并重新校验 manifest 和资源、展示源码审阅。**当前 AnyWhere 的发现页仍使用 GitHub `anywhere-pack` topic；切换目录来源及固定提交安装属于后续客户端接入。** 示例现在就可通过现有 GitHub 导入入口输入 `appdev/anywhere-tool-chain-demo` 使用。
